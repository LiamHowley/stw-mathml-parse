...
(define-test parse-mathml...
  ...
  (of-type 'math (get-element-with-attribute *parsed-mathml* "display"))
...
(define-test reader...
  ...
  (let* ((document-node (read-from-string "<mfrac linethickness=\"0\"><mi>n</mi><mi>k</mi></mfrac>"))
    ...
    (true (slot-exists-p child-node 'linethickness))
    (is string= "0" (slot-value child-node 'linethickness))
